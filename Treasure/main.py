...
def proximo_estagio():
    ...
    if estagio_atual >= len(ESTAGIOS):
        mostrar_mensagem("PARABÉNS!", VERDE, 3000)
        mostrar_mensagem("VOCÊ COMPLETOU! ", VERDE, 3000)
...
def desenhar_botoes():
    ...
    TELA.blit(texto_estagio, (LARGURA // 2 - 50, ALTURA - 20))


def mostrar_mensagem(mensagem, cor, tempo=2000):
    
    tamanho_fonte = int(ALTURA * 0.08)
    fonte_dinamica = pygame.font.SysFont(None, tamanho_fonte)

    texto = fonte_dinamica.render(mensagem, True, cor)

    
    largura_caixa = LARGURA * 0.8
    altura_caixa = ALTURA * 0.3
    x_caixa = (LARGURA - largura_caixa) // 2
    y_caixa = (ALTURA - altura_caixa) // 2

    
    pygame.draw.rect(TELA, BRANCO, (x_caixa, y_caixa, largura_caixa, altura_caixa))
    pygame.draw.rect(TELA, PRETO, (x_caixa, y_caixa, largura_caixa, altura_caixa), 3)

    
    texto_x = LARGURA // 2 - texto.get_width() // 2
    texto_y = ALTURA // 2 - texto.get_height() // 2
...
def main():
    ...
    while rodando:
        ...
        if tentativas == 0 and matriz[tesouro_x][tesouro_y] != "T":
            ...
            rodando = False
    while True:
        mostrar_mensagem("Pressione R para reiniciar", AZUL, 500)
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
